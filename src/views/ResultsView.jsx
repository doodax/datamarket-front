...
export default function ResultsView({ role }) {
  ...
  // L'enseignant·e a lancé la mission suivante : retour à la vue de jeu
  // Ce hook DOIT être avant tout return conditionnel (règle des hooks React)
  useEffect(() => {
    if (!sessionState) return;
    if (role === 'group' && (sessionState.state === 'setup' || sessionState.state === 'running')) {
      navigate(`/group/${code}`, { replace: true });
    }
  }, [sessionState, role, code, navigate]);
  ...
  if (sessionState.state !== 'revealed' || !reports) {
    // État "locked" : rapports générés, on attend la révélation
    if (sessionState.state === 'locked') {
      return (
          <div className="min-h-screen flex items-center justify-center">
            <div className="text-center max-w-md">
              <div className="text-terminal-amber mb-2 font-mono text-sm uppercase tracking-widest animate-pulse">
    ...
    // État "setup" ou "running" : mission suivante — redirection gérée par useEffect ci-dessus
    if (role === 'group') {
      return (
          <div className="min-h-screen flex items-center justify-center">
            <div className="text-center">
              <div className="text-terminal-cyan text-4xl mb-4 animate-pulse">→</div>
              <div className="text-ink-100 text-xl font-display mb-2">Mission suivante en cours…</div>
              <div className="text-ink-400 text-sm">Redirection automatique dans un instant.</div>
            </div>
          </div>
      );
    }
